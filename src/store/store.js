...
import { SERVER_URL, CLIENT_URL, WG_APP_ID } from '../utils/config';
...
const store = new Vuex.Store({
  ...
  actions: {
    ...
    WARGAMMING_LOGIN: async (ctx) => {
      ...
      const res = await axios.get(url, {
        params: {
          ...
          nofollow: 1,
          redirect_uri: urlJoin(CLIENT_URL, '/auth/wargaming/success')
        }
      });
    ...
    WARGAMMING_REFRESH_TOKEN: async (ctx, user) => {
      const url = urlJoin(SERVER_URL, '/api/auth/prolongate/');
